 ...
 */
abstract class CommonControllerTestCase extends CommonTestCase {
 ...
 */
	...
	public function getMockForController($classname, $methods = array(), $args = array(), $options = array(), $components = array(), $models = array()) {
		...
		if (empty($args)) {
			$args = array(new CakeRequest, $this->getMock('CakeResponse'));
		}

		$defaultOptions = array(
			'setRequestParams' => true,
			'constructClasses' => true,
			'mockAuthComponent' => true,
			'disableSecurityComponent' => true,
		);

		$options = array_merge($defaultOptions, (array) $options);

		$ControllerMock = $this->getMock(Inflector::classify($classname), $methods, $args);

		if ($options['setRequestParams']) {
			list($plugin, $controller) = pluginSplit($classname);
			$ControllerMock->request->params = array(
				'pass' => array(),
				'named' => array(),
				'controller' => str_replace('_controller', '', Inflector::underscore($controller)),
				'plugin' => Inflector::underscore($plugin),
				'url' => array(),
			);
			Router::setRequestInfo($ControllerMock->request);
		}

		if (!$options['constructClasses']) {
			return $ControllerMock;
		}

		$ControllerMock->constructClasses();
		...
		foreach ($models as $model => $methods) {
			...
			list($plugin, $alias) = pluginSplit($model, true);

			if (!empty($methods['alias'])) {
				$alias = $methods['alias'];
				unset($methods['alias']);
			}

			if (empty($methods) || true === $methods) {
				$methods = array();
			}

			$ControllerMock->{$alias} = $this->getMockForModel($plugin . $model, $methods);
		}

		if (false !== $options['mockAuthComponent'] && $ControllerMock->Components->loaded('Auth')) {
			$authMethods = $options['mockAuthComponent'];
			if (is_string($options['mockAuthComponent'])) {
				$authMethods = array($options['mockAuthComponent']);
			} else if (!is_array($options['mockAuthComponent'])) {
				$authMethods = array('user');
			}
			$this->Auth = $this->getMock('AuthComponent', $authMethods, array($ControllerMock->Components, $ControllerMock->components['Auth']));
			$ControllerMock->Auth = $this->Auth;
		}

		if ($ControllerMock->Components->loaded('Security')) {
			$ControllerMock->Components->unload('Security');
		}
		...
		foreach ($components as $component => $methods) {
			...
			if (!$ControllerMock->Components->loaded($component)) {
				continue;
			}

			if (false === $methods) {
				$ControllerMock->Components->unload($component);
				continue;
			}

			$settings = array();
			if (!empty($ControllerMock->components[$component])) {
				$settings = $ControllerMock->components[$component];
			}

			$ControllerMock->{$component} = $this->getMock($component . 'Component', (array) $methods, array($ControllerMock->Components, $settings));
		}
 ...
 */
	protected function _setPost($data = array()) {
		...
		$controllerName = str_replace('ControllerTest', '', get_class($this));
		$this->{$controllerName}->request->data = array_merge($data, array('_method' => 'POST'));
	}
